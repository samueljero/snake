...
#ifndef FREEZE_TIME_SYNCHRONIZER_H
#define FREEZE_TIME_SYNCHRONIZER_H
 ...
 *
 *   DefaultValue::Bind ("Synchronizer", "FreezeTimeSynchronizer");
 *
 ...
 */
class FreezeTimeSynchronizer : public Synchronizer
{
public:
  FreezeTimeSynchronizer ();
  virtual ~FreezeTimeSynchronizer ();
  int ToggleFreeze(bool);
 ...
 */
  ...
  bool IsTimeFrozen(void);


  void StartFreeze(void);
  void EndFreeze(void);

  uint64_t m_startFreeze; //realtime associated with start of time freeze
  uint64_t m_frozenTime;  //amount of realtime passed during frozen time
...
#endif /* FREEZE_TIME_SYNCHRONIZER_H */
