...
NS_LOG_COMPONENT_DEFINE ("FreezeTimeSynchronizer");

namespace ns3 {

FreezeTimeSynchronizer::FreezeTimeSynchronizer ()
{
...
bool
FreezeTimeSynchronizer::DoRealtime (void)
{
  NS_LOG_FUNCTION_NOARGS ();
  return true;
}
...
uint64_t
FreezeTimeSynchronizer::DoGetCurrentRealtime (void)
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
FreezeTimeSynchronizer::DoSetOrigin (uint64_t ns)
{
  NS_LOG_FUNCTION_NOARGS ();
//
...
int64_t
FreezeTimeSynchronizer::DoGetDrift (uint64_t ns)
{
  NS_LOG_FUNCTION_NOARGS ();
//
// In order to make sure we're really locking the simulation time to some 
...
int 
FreezeTimeSynchronizer::ToggleFreeze(bool freeze) {
	NS_LOG_FUNCTION_NOARGS ();
	// simple error check just for the case
	if (freeze && IsTimeFrozen()) return -1;
	if (!freeze && !IsTimeFrozen()) return -1;
	if (!IsTimeFrozen()) {
		NS_LOG_INFO("Start Freeze");
		StartFreeze();
	} else {
		NS_LOG_INFO("End Freeze");
		EndFreeze();
	}
	return 1;
}
...
bool
FreezeTimeSynchronizer::IsTimeFrozen(void) {
	NS_LOG_FUNCTION_NOARGS ();
	if (m_startFreeze == 0) {
		return false;
	} else {
...
void
FreezeTimeSynchronizer::StartFreeze(void) {
	NS_LOG_FUNCTION_NOARGS ();
	m_startFreeze = GetRealtime();
	NS_LOG_INFO("start Freeze: " << m_startFreeze);
}

void
FreezeTimeSynchronizer::EndFreeze(void) {
	NS_LOG_FUNCTION_NOARGS ();
	m_frozenTime += GetRealtime() - m_startFreeze;
	NS_LOG_INFO("frozen Time: " << m_frozenTime);
	m_startFreeze = 0;
}
...
bool
FreezeTimeSynchronizer::DoSynchronize (uint64_t nsCurrent, uint64_t nsDelay)
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
FreezeTimeSynchronizer::DoSignal (void)
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
FreezeTimeSynchronizer::DoSetCondition (bool cond)
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
FreezeTimeSynchronizer::DoEventStart (void)
{
  NS_LOG_FUNCTION_NOARGS ();
...
uint64_t
FreezeTimeSynchronizer::DoEventEnd (void)
{
  NS_LOG_FUNCTION_NOARGS ();
...
bool
FreezeTimeSynchronizer::SpinWait (uint64_t ns)
{
  NS_LOG_FUNCTION_NOARGS ();
//
...
bool
FreezeTimeSynchronizer::SleepWait (uint64_t ns)
{
  NS_LOG_FUNCTION_NOARGS ();
//
...
// set true by a call to FreezeTimeSynchronizer::SetCondition (true) followed
// by a call to FreezeTimeSynchronizer::Signal().  In either case, we are done
...
uint64_t
FreezeTimeSynchronizer::DriftCorrect (uint64_t nsNow, uint64_t nsDelay)
{
...
uint64_t
FreezeTimeSynchronizer::GetRealtime (void)
{
...
uint64_t
FreezeTimeSynchronizer::GetNormalizedRealtime (void)
{
  if (IsTimeFrozen()) {
    return m_startFreeze;
    //return m_startFreeze - m_realtimeOriginNano - m_frozenTime;
  }
  return GetRealtime () - m_realtimeOriginNano - m_frozenTime;
}

void
FreezeTimeSynchronizer::NsToTimeval (int64_t ns, struct timeval *tv)
{
...
uint64_t
FreezeTimeSynchronizer::TimevalToNs (struct timeval *tv)
{
...
void
FreezeTimeSynchronizer::TimevalAdd (
  struct timeval *tv1, 
  struct timeval *tv2,
