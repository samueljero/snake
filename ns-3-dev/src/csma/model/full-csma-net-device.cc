...
Ipv4Header CsmaNetDevice::GetIPHeader(Ptr<Packet> packet_received)
{
...
bool 
CsmaNetDevice::GhostIntercept(Ptr<Packet> packet_received, bool sending, bool *writeToTap)
{
  ...
  if (protocol == UdpL4Protocol::PROT_NUMBER && !sending) {
	...
	*/
	...
	if (protocol == UdpL4Protocol::PROT_NUMBER) {
		...
		//std::cout << "PACKET GOES TO UDP port " << destPort << std::endl;
	}
	...
	if (mtag.GetSrcNode() != m_node->GetId()) { // from outside - intercept
		...
		if (mtag.GetMalStatus() == MaliciousTag::FROMTAP) {
			NS_LOG_INFO(" Intercept packet from my TAP: later change IP dest from "
					<< ipHeader.GetDestination() << " to " << ipHeader.GetSource()); // from my tap device. should intercept
			mtag.SetSpoof(true);	
		}
		else {
	...
	if (m_node->IsMalicious()) { 
		...
		m_rxCallback (this, packet, 2048, header.GetSource ()); 
...
{
  NS_LOG_FUNCTION (packet << senderDevice << " at node " << m_node->GetId());
  NS_LOG_LOGIC ("UID is " << packet->GetUid ());
	MaliciousTag mtag;
	NS_LOG_INFO(" RECEIVE PACKET " << packet->GetUid());
  ...
  // HJLEE: if it's from my proxy, it can be valid 
  if (senderDevice == this) {
		if (!m_node->IsMalicious() ||
				!(mtag.GetMalStatus() == MaliciousTag::FROMPROXY && m_node->IsLocalAddress(GetIPHeader(packet).GetDestination())))
    {
			NS_LOG_INFO("DISCARD LOOP");
      return;
		}
	}
  ...
  if (header.GetLengthType () <= 1500)
	...
	/* Ghost Intercept incoming packets */
	if (m_node->IsMalicious() && protocol == 2048) {
		if (GhostIntercept(originalPacket, false, &writeToTap)) return; // this should be receiving a pcket expect tag 3
	}
  ...
  PacketType packetType;
	...
	if (protocol == 2048
		&& m_node->IsLocalAddress(GetIPHeader(originalPacket).GetDestination())) {
		writeToTap = true;
	}
...
CsmaNetDevice::SendFrom (Ptr<Packet> packet, const Address& src, const Address& dest, uint16_t protocolNumber)
{
  ...
  NS_LOG_LOGIC ("UID is " << packet->GetUid () << " at node " << m_node->GetId());
  ...
  Mac48Address source = Mac48Address::ConvertFrom (src);
	if (m_node->IsMalicious()) {
		if (mtag.GetMalStatus() == MaliciousTag::FROMPROXY) {
			if (protocolNumber == 2048) {
				// XXX - LOOK AT HERE. the current setting doesn't allow to connect, but why?
				// actually initiator who doesn't use Proxy should just pass the packet to the tap
				//if (mtag.GetSrcNode() != m_node->GetId()) { // XXX - this is working
				if (mtag.GetSrcNode() != m_node->GetId() || (mtag.GetSrcNode() == m_node->GetId() && m_node->IsLocalAddress(mtag.GetIPDestination()))) {
						NS_LOG_INFO(" WRITE TO TAP : " ); // this should be received and forwarded to tap
						mtag.Print(std::cerr);
						AddHeader (packet, destination, source, protocolNumber);
						Receive(packet, this); // XXX - should we call receive to make things clear?
						return true;
				} else {
					NS_LOG_INFO(" GO OUT");
					mtag.StopTimer("go out");
				}
			}
		} 
	}
	...
	if (m_node->IsMalicious()) {
		bool writeToTap = false;
		if (protocolNumber == 2048) {
			if (GhostIntercept(packet, true, &writeToTap)) return true;
		}
		if (writeToTap) NS_LOG_INFO(" DID NOT EXPECT WRITE TO TAP");
		NS_LOG_INFO(" GHOST DO NOT INTERCEPT");
	}
